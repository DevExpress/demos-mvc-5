...
public static class AirlineDataProvider {
    const string
        FlightsSessionName = "DX_Flights",
        DependenciesSessionName = "DX_FlightDependencies",
        PlanesSessionName = "DX_Planes",
        AssignmentsSessionName = "DX_Flight_Planes_Assignmnents";
    ...
    public static List<Flight> Flights {
        get {
            if(Session[FlightsSessionName] == null)
                Session[FlightsSessionName] = CreateFlights();
            else
                UpdateFlightsProgress();
            return (List<Flight>)Session[FlightsSessionName];
        }
    }
    public static List<FlightDependency> Dependencies {
        get {
            ...
            return (List<FlightDependency>)Session[DependenciesSessionName];
        }
    }
    public static List<Plane> Planes {
        get {
            if(Session[PlanesSessionName] == null)
                Session[PlanesSessionName] = CreatePlanes();
            return (List<Plane>)Session[PlanesSessionName];
        }
    }
    public static List<Assignment> Assignments {
        get {
            if(Session[AssignmentsSessionName] == null)
                Session[AssignmentsSessionName] = CreateAssignments();
            return (List<Assignment>)Session[AssignmentsSessionName];
        }
    }
    ...
    static void UpdateFlightsProgress() {
        var flights = Session[FlightsSessionName] as List<Flight>;
        if(flights != null) {
