...
namespace DevExpress.Web.Demos
{
	...
	public class DemoHelper
	{
		...
		const string DarkThemeName = "Office365Dark";
		const string MaterialCompactOrangeTheme = "MaterialCompactOrange";
		...
		public string Theme {
			get {
				return DevExpress.Web.ASPxWebControl.GlobalTheme != DarkThemeName ? MaterialCompactOrangeTheme : DarkThemeName;
			}
		}
		...
		public void PrepareFLGroup(LayoutGroup group, bool isOptionFormLayout, ControlOptionsSettings? settings) {
			...
			if(settings != null) {
				...
				group.GridSettings.ChangeCaptionLocationAtWidth = optionsValue.ColumnMinWidth;
			}
		}
		public void PrepareFLItem(LayoutItemBase itemBase, bool isOptionFormLayout) {
			...
			foreach(var rawControl in controls) {
				...
				if(control is ASPxCheckBox)
					layoutItem.CaptionSettings.ChangeCaptionLocationInAdaptiveMode = DefaultBoolean.False;
			}
		}
