...
namespace DevExpress.Web.Demos {
	public class BootstrapDemoItemBase : Panel {
		public BootstrapDemoItemBase() {
			IECompatible = true;
		}
		public bool IECompatible { get; set; }
		protected bool IsContentIncompatibleWithBrowser { get { return !IECompatible && RenderUtils.Browser.IsIE; } }
		protected override void OnPreRender(EventArgs e) {
			if(IsContentIncompatibleWithBrowser)
				Controls.Clear();
			else
				base.OnPreRender(e);
		}
		protected override void RenderContents(HtmlTextWriter writer) {
			if(IsContentIncompatibleWithBrowser)
				RenderBrowserIncompatibilityInfo(writer);
			else
				base.RenderContents(writer);
		}
		protected virtual void RenderBrowserIncompatibilityInfo(HtmlTextWriter writer) {
			writer.AddAttribute(HtmlTextWriterAttribute.Class, "incompatibility-info table-bordered");
			writer.RenderBeginTag(HtmlTextWriterTag.Div);
			{
				if(BootstrapUtils.BootstrapVersion > 3)
					writer.AddAttribute(HtmlTextWriterAttribute.Class, "d-table-cell");
				writer.RenderBeginTag(HtmlTextWriterTag.Div);
				{
					writer.WriteLine(FileUtils.GetResourceFileText(typeof(BootstrapDemoItemBase).Assembly, "Data.Icons.info.svg"));
				}
				writer.RenderEndTag();
				if(BootstrapUtils.BootstrapVersion > 3)
					writer.AddAttribute(HtmlTextWriterAttribute.Class, "d-table-cell w-100");
				writer.RenderBeginTag(HtmlTextWriterTag.Div);
				{
					writer.RenderBeginTag(HtmlTextWriterTag.Div);
					{
						...
						{
							writer.WriteEncodedText("Unsupported Web Browser");
						}
						writer.RenderEndTag();
						writer.RenderBeginTag(HtmlTextWriterTag.P);
						{
							writer.WriteEncodedText("To explore the capabilities of the DevExpress product line, please reload this webpage with a different web browser.");
						}
						writer.RenderEndTag();
					}
					writer.RenderEndTag();
	...
	public class BootstrapDemoItem : BootstrapDemoItemBase {
		...
		protected void RenderLanguageContent(HtmlTextWriter writer, string dataCode, string codeClass, string code, bool isActive) {
			...
			writer.Write($"<pre><code class=\"{className}\" data-code=\"{dataCode}\">{code}</code></pre>");
		}
	}
