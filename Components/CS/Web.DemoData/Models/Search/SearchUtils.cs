	...
	public static class SearchUtils {
		...
		static IEnumerable<SearchResult> DoSearch(List<string[]> requests, DemoProductModel product) {
			...
			if(product.Intro != null)
				results.AddRange(GetRes(requests, product.Intro, string.Format("{0}", SearchAgregator.HighlightOccurences(product.NavItemTitle, requests, true)), product.Intro.Product.Title.ToUpper()));
			foreach(var group in product.GetVisibleGroups()) {
				foreach(var demo in group.GetVisibleDemos())
		...
		public static Dictionary<string, int> GetKeywordsRankList(ModelBase model) {
			...
			var intro = model as IntroPageModel;
			if(product != null) {
				...
				if(intro != null && intro.Product != null)
					textRanks.Add(new TextRank(intro.Product.Title, 15));
				else
