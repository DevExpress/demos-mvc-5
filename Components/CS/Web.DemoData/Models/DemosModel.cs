...
namespace DevExpress.Web.Demos {
	...
	public class DemosModel {
		...
		public static DemosModel CreateInstance(bool isMvc) {
			...
			using(Stream stream = GetDemosModelContentStream(isMvc)) {
				instance = SafeXml.Deserialize<DemosModel>(stream);
			}
			foreach(var demoProduct in instance.DemoProducts) {
				foreach(var group in demoProduct.Groups) {
					foreach(var demo in group.GetAllDemos(false)) {
						demo.Group = group;
	...
	public class DemoProductModel : ModelBase {
		...
		public List<DemoGroupModel> GetVisibleGroups() {
			return Groups.Where(g => !g.HideNavItem).ToList();
		}
		[XmlAttribute]
		...
		List<DemoPageModel> CreateHighlightedDemos() {
			...
			foreach(DemoGroupModel group in GetVisibleGroups()) {
				foreach(DemoPageModel demo in group.GetVisibleDemos()) {
