...
namespace DevExpress.Web.Demos {
	...
	public static class Utils {
		internal const string
			...
			CorpColor = "#F78119";
		static string[] TreeControlNoTreeLinesThemes = new string[] { "DevEx", "Metropolis", "MetropolisBlue", "Moderno", "Mulberry", "iOS", "Material", "MaterialCompact", "Office2010Blue", "Office2010Black", "Office2010Silver", "Office365", "Office365Dark" };
		static string[] TreeListNoGridLinesThemes = new string[] { "DevEx", "Office2010Blue", "Office2010Black", "Office2010Silver" };
		...
		public static DemoPageModel CurrentDemoPage {
			get {
				return !IsIntro ? CurrentDemo as DemoPageModel : null;
			}
		}
		...
		static string[] GetHighlightedTagNames(DemoPageModel demo) {
			var highlightedTagNames = demo.Group.GetHighlightedTagNames().
		...
		static bool HasCodeFile(string url) {
			...
			return Regex.IsMatch(text, @"<[^>]*\bCodebehind\s*=\s*""[\w\.]+\""[^>]*>");
		}
		...
		public static void EnsureRequestValidationMode() {
			try {
				if(Environment.Version.Major >= 4) {
					Type type = DevExpress.Data.Internal.SafeTypeResolver.GetKnownType(typeof(WebControl).Assembly, "System.Web.Configuration.RuntimeConfig");
...
#pragma warning restore DX0025
		...
		public static bool CanChangeTreeControlTreeLines(){
			return !TreeControlNoTreeLinesThemes.Contains(ASPxWebControl.GlobalTheme);
		}
		public static bool CanChangeTreeListGridLines(){
			return !TreeListNoGridLinesThemes.Contains(ASPxWebControl.GlobalTheme);
		}
		public static string GetInfoMessageForTreeControlNoTreeLinesThemes() {
			return GetInfoMessageForTreeControl(true);
		}
		public static string GetInfoMessageForTreeListNoGridLinesThemes() {
			return GetInfoMessageForTreeControl(false);
		}
		static string GetInfoMessageForTreeControl(bool forTreeLines) {
			var excludedThemes = forTreeLines ? TreeControlNoTreeLinesThemes : TreeListNoGridLinesThemes;
			var infoFeatures = forTreeLines ? "tree lines" : "grid lines";
			var infoFeature = forTreeLines ? "tree line" : "grid line";
			var themesInfos = ThemesModel.Current.GetAllThemes();
#pragma warning disable DX0025 // not a path (and no traversal here)
			var result = String.Join(", ", themesInfos.Where(x => excludedThemes.Contains(x.Name)).Select(x => String.Format("<b>{0}</b>", x.Title)));
#pragma warning restore DX0025
			return HttpUtility.HtmlEncode(
					String.Format("By design, the {0} themes do not display {1}. Switch to a different theme to view and manipulate {2} setting in this demo.", result, infoFeatures, infoFeature)
				);
		}
	}
