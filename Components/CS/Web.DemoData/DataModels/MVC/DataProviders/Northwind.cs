...
namespace DevExpress.Web.Demos.Mvc {
	public static class NorthwindDataProvider {
		const char gridMultipleKeySeparator = '|';
		...
		public static IList<EditableOrderDetailExtended> GetEditableOrderDetailsExtended() {
			IList<EditableOrderDetailExtended> orders = HttpContext.Current.Session["OrderDetailsExtended"] as IList<EditableOrderDetailExtended>;
			if (orders == null) {
				orders = (from order in DB.Order_Details_Extendeds
						  select new EditableOrderDetailExtended {
				...
				HttpContext.Current.Session["OrderDetailsExtended"] = orders;
			}
			return orders;
		}
		public static EditableOrderDetailExtended GetEditableOrderDetailExtended(string orderIDProductID) {
			string[] ids = orderIDProductID.Split(gridMultipleKeySeparator);
			var orderID = Convert.ToInt32(ids[0]);
			var productID = Convert.ToInt32(ids[1]);
			return GetEditableOrderDetailsExtended().FirstOrDefault(o => o.OrderID == orderID && o.ProductID == productID);
		}
		public static void DeleteOrderDetailExtended(string orderIDProductID) {
			var order = GetEditableOrderDetailExtended(orderIDProductID);
			if (order != null)
				GetEditableOrderDetailsExtended().Remove(order);
		}
		public static void UpdateOrderDetailExtended(EditableOrderDetailExtended order) {
			var editOrder = GetEditableOrderDetailExtended(string.Join(gridMultipleKeySeparator.ToString(), order.OrderID, order.ProductID));
			if (editOrder != null) {
				editOrder.UnitPrice = order.UnitPrice;
		...
		public static IList<EditableEmployee> GetEditableEmployees() {
			const string SessionName = "DXDemoEmployees";
			IList<EditableEmployee> employeesList = HttpContext.Current.Session[SessionName] as IList<EditableEmployee>;
			if(employeesList == null) {
				employeesList = (from employee in DB.Employees
				...
				HttpContext.Current.Session[SessionName] = employeesList;
			}
		...
		public static IList<EditableCustomer> GetEditableCustomers() {
			const string SessionName = "DXDemoCustomers";
			IList<EditableCustomer> customersList = HttpContext.Current.Session[SessionName] as IList<EditableCustomer>;
			if(customersList == null) {
				...
				HttpContext.Current.Session[SessionName] = customersList;
			}
	...
	public class EditableOrderDetailExtended {
		public int OrderID { get; set; }
		public int ProductID { get; set; }
		public string ProductName { get; set; }
		public decimal? UnitPrice { get; set; }
		[Range(0, 1000, ErrorMessage = "Must be between 0 and 2000")]
	...
	public class Invoice {
		...
		public DateTime OrderDate {
			get { return InvoiceDate.HasValue ? new DateTime(2021, InvoiceDate.Value.Month, InvoiceDate.Value.Day) : DateTime.Now; }
		}
