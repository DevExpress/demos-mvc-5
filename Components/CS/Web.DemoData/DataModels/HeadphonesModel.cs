...
namespace DevExpress.Web.Demos {
	...
	public class HeadphonesDataProvider {
		...
		static string PhotoUrl(int id, string postfix) {
#pragma warning disable DX0025 // constructed from const data (and no traversal here)
		...
		static List<Headphone> CreateHeadphones() {
			lock(createLocker) {
				...
				using(FileStream fs = File.OpenRead(DataFilePath)) {
					...
					list = SafeXml.Deserialize<List<Headphone>>(sr.ReadToEnd());
				}
