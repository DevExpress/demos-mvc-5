...
namespace DevExpress.Web.Demos {
	public static class DatabaseGenerator {
		...
		public class FieldData {
			static NonCryptographicRandom rnd = NonCryptographicRandom.Default;
			public delegate object ValueGeneratorDelegate(NonCryptographicRandom rnd);
...
#pragma warning disable DX0024 // The Insert command is parameterized and doesn't depend on user input
		...
		static int GetRecordCount(string tableKey) {
			try {
				...
				using(SqlConnection connection = new SqlConnection(table.ConnectionString)) {
					connection.Open();
#pragma warning disable DX0024
					using(var cmd = new SqlCommand("select count(*) from [" + table.Name + "]", connection)) 
