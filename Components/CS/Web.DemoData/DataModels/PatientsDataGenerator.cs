...
namespace DevExpress.Web.Demos {
	public static class PatientsDataGenerator {
		...
		static IList GenerateInMemory(int count) {
			...
			var durationField = new DatabaseGenerator.FieldData("Duration", GenerateDuration);
			return Enumerable.Range(0, count).Select(i => new {
				...
				Duration = Convert.ToInt32(durationField.GenerateValue())
			}).ToList();
		}
		static object GenerateDuration(NonCryptographicRandom rnd) {
			return rnd.Next(20, 120);
		}
		static object GenerateVisitDate(NonCryptographicRandom rnd) {
